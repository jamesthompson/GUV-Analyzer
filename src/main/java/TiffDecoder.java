...
/**
 * Decodes single and multi-frame TIFF files.
 */
...
public class TiffDecoder {
    ...
    /**
     * Save the image description in the specified FileInfo. ImageJ saves
     * spatial and density calibration data in this string. For stacks, it also
     * saves the number of images to avoid having to decode an IFD for each
     * image.
     */
    ...
    /**
     * Returns a double containg the value represented by the specified
     * <code>String</code>.
     *
     * @param s the string to be parsed.
     * @param defaultValue the value returned if
     * <code>s</code> does not contain a parsable double
    ...
    void dumpTag(int tag, int count, int value, FileInfo fi) {
        ...
        String nametag = getName(tag);
        String cs = (count == 1) ? "" : ", count=" + count;
        dInfo += "    " + tag + ", \"" + nametag + "\", value=" + lvalue + cs + "\n";
    ...
    String getName(int tag) {
        ...
        switch (tag) {
            ...
            case IMAGE_DESCRIPTION:
                nametag = "ImageDescription";
                break;
            case BITS_PER_SAMPLE:
                nametag = "BitsPerSample";
                break;
            case SAMPLES_PER_PIXEL:
                nametag = "SamplesPerPixel";
                break;
            case ROWS_PER_STRIP:
                nametag = "RowsPerStrip";
                break;
            case STRIP_BYTE_COUNT:
                nametag = "StripByteCount";
                break;
            case X_RESOLUTION:
                nametag = "XResolution";
                break;
            ...
            case ARTEST:
                ...
                break;
            case HOST_COMPUTER:
                nametag = "HostComputer";
                break;
            case PLANAR_CONFIGURATION:
                nametag = "PlanarConfiguration";
                break;
            case COMPRESSION:
                nametag = "Compression";
                break;
            ...
            case COLOR_MAP:
                ...
                break;
            case SAMPLE_FORMAT:
                nametag = "SampleFormat";
                break;
            case JPEG_TABLES:
                nametag = "JPEGTables";
                break;
            case NIH_IMAGE_HDR:
                nametag = "NIHImageHeader";
                break;
            case META_DATA_BYTE_COUNTS:
                nametag = "MetaDataByteCounts";
                break;
            case META_DATA:
                nametag = "MetaData";
                break;
    ...
    public ArrayList<FileInfo> getTiffInfo() throws IOException {
        long ifdOffset;
        if (in == null) {
        ...
        if (info.isEmpty()) {
            ...
            if (url != null) {
                in.seek(0);
                info.get(0).inputStream = in;
            } else {
                in.close();
            }
            if (info.get(0).info == null) {
                info.get(0).info = tiffMetadata;
            }
            return info;
        }
    }
